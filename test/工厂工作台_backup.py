...
def import_jobs(fp, batch_name):
    ...
    for r in range(2, ws.max_row + 1):
        ...
        c.execute('INSERT INTO job_items (batch_id, sku, product_name, qty, job_number) VALUES (?,?,?,?,?)', (bid, sku.upper(), name, qty, job_no))
        cnt += 1
...
.cr{font-size:11px;color:#999;text-align:center;padding:12px;margin-top:8px}
	...
	.pri-item{background:#fffaf0;border-left:3px solid #e53e3e;margin-bottom:6px!important}</style></head><body>
				<div class="hd"><div style="display:flex;align-items:center;gap:8px"><h1 style="flex:1">🔧 车间加工列表 <span style="font-size:11px;color:#a0aec0;font-weight:400">'''+VERSION+'''</span></h1><a href="#" style="color:#fff;text-decoration:none;font-size:18px" onclick="showQR();return false">📱</a></div><p id="batchInfo">全部加工单</p></div>
    ...
    var ti=items;
	    ...
	    items.forEach(function(i){
	        ...
	        var priBadge = i.priority ? '<span class="pri-badge">⭐优先</span>' : '';
		        var jobNoHtml = i.job_number ? '<span style="font-size:10px;color:#718096">#'+i.job_number+' </span>' : '';
		        li.innerHTML='<div class="top"><span class="st '+stClass+'"></span>'+jobNoHtml+'<span class="sk">'+i.sku+priBadge+'</span><span style="font-size:10px;color:#999">'+statusLabel[i.status]+'</span></div>'+'<div class="nm">'+(i.name||'')+'</div>'+info+'<div class="btns">'+btns+'</div>';
		        ul.appendChild(li);
		    });
...
async function doCalc(){
    ...
    try{
        ...
        var d=await r.json();
        if(d.status==='ok'){
            var html='✅ <b>预计耗时：'+d.hours+' 小时</b><br>';
	...
	.calc-res.err{background:#ffebee;border:1px solid #ef9a9a;color:#b71c1c;display:block}</style></head><body>
		<div class="hd"><div class="hd-top">
			<div style="display:flex;align-items:center;gap:10px"><a href="/" style="color:#fff;text-decoration:none;font-size:14px;font-weight:600;background:rgba(255,255,255,.15);padding:4px 10px;border-radius:6px;white-space:nowrap">🏠 主页</a><div><h1>🔧 车间看板 <span style="font-size:11px;color:#a0aec0;font-weight:400">'''+VERSION+'''</span></h1><div class="sub" id="batchInfo">加载中...</div></div></div>
						<div class="people-bar"><button class="calc-btn" onclick="showCalc()">⚡产能</button><a class="nav-link" href="/workshop" target="_blank">📱列表</a><a class="nav-link" href="/workshop_admin" target="_blank">🛡管理</a></div>
		</div></div>
...
function renderCol(elId,items,st){
    ...
    items.forEach(function(i){
        ...
        var jobNoHtml3 = i.job_number ? '<div style="font-size:10px;color:#718096;padding:0 18px">📋 #'+i.job_number+'</div>' : '';
