...
.empty{text-align:center;font-size:12px;color:#a0aec0;padding:16px 0}
	...
	.calc-res.err{background:#ffebee;border:1px solid #ef9a9a;color:#b71c1c;display:block}</style></head><body>
		<div class="hd"><div class="hd-top">
			<div style="display:flex;align-items:center;gap:10px"><a href="/" style="color:#fff;text-decoration:none;font-size:14px;font-weight:600;background:rgba(255,255,255,.15);padding:4px 10px;border-radius:6px;white-space:nowrap">🏠 主页</a><div><h1>🔧 车间看板 <span style="font-size:11px;color:#a0aec0;font-weight:400">'''+VERSION+'''</span></h1><div class="sub" id="batchInfo">加载中...</div></div></div>
							<div class="people-bar"><button class="calc-btn" onclick="showCalc()">⚡产能</button><button class="calc-btn" style="background:#38a169" onclick="showEta()">📝录入</button><button class="calc-btn" style="background:#4361ee" onclick="showEtaQuery()">🔍产能查询</button><a class="nav-link" href="/workshop" target="_blank">📱列表</a><a class="nav-link" href="/workshop_admin" target="_blank">🛡管理</a></div>
		</div></div>
...
// \u8de8\u9875\u9762\u5b9e\u65f6\u540c\u6b65
try{
	var boardSync = new BroadcastChannel('workshop_sync');
	boardSync.onmessage = function(e) { if (e.data === 'refresh') loadBoard(); };
	window.addEventListener('storage', function(e) { if (e.key === 'default_people') loadBoard(); });
}catch(e){}
...
def _html_response(s):
    return Response(s, content_type='text/html;charset=utf-8')
...
# ---- Static page routes ----
@app.route('/')
def route_index(): return _html_response(HTML_MAIN)
@app.route('/scan')
def route_scan(): return _html_response(SCAN_PAGE)
@app.route('/scan_admin')
def route_scan_admin(): return _html_response(SCAN_ADMIN)
@app.route('/scan_history_page')
def route_scan_history_page(): return _html_response(SCAN_HISTORY)
@app.route('/workshop')
def route_workshop(): return _html_response(WORKSHOP_PAGE)
@app.route('/workshop_board')
def route_workshop_board(): return _html_response(WORKSHOP_BOARD)
@app.route('/workshop_admin')
def route_workshop_admin(): return _html_response(WORKSHOP_ADMIN)
@app.route('/diag')
def route_diag(): return _html_response(DIAG_PAGE)
@app.route('/us')
def route_us(): return _html_response(US_PAGE)
@app.route('/cards')
def route_cards(): return _html_response(CARDS_PAGE)
@app.route('/pt')
def route_pt(): return _html_response(POST_TEST_PAGE)
@app.route('/health')
def route_health(): return _json_response({'status':'ok','port':PORT})
@app.route('/get_ip')
def route_get_ip(): return _json_response({'ip':get_ip()})
...
def route_scan_info():
    q = request.args
    bids = q.get('batch', '0')
    bid = int(bids) if bids.isdigit() else None
    if not bid:
        b = get_latest_batch()
        if b: bid = b[0]
    if bid:
        ...
        if batch:
            ...
            for rg in regions:
                region_stats[rg] = get_region_stats(bid, rg)
            conn.close()
            return _json_response({'batch_name':batch[0],'total':batch[2],'regions':regions,'region_stats':region_stats})
        conn.close()
    return _json_response({'batch_name':'\u6682\u65e0\u6279\u6b21','total':0,'regions':[],'region_stats':{}})

@app.route('/scan_check')
def route_scan_check():
    q = request.args
    code = q.get('code','').strip(); worker = q.get('worker','').strip(); region = q.get('region','').strip()
    bid = get_latest_batch()
    if not bid: return _json_response({'found':False,'history':[],'message':'\u6682\u65e0\u6279\u6b21\u6570\u636e'})
    ...
    if item and item[0] != 'wrong_region':
        ...
        h = [{'code':h[0],'worker':h[1],'status':'\u2713' if h[2]=='correct' else '\u2717','qty':h[3],'time':h[4],'note':h[6] if h[6] else ''} for h in c.fetchall()]
        conn.close()
        return _json_response({'found':True,'region':item[1],'batch':bid[0],'message':'\u2713 '+code+' - '+item[1]+'('+str(item[2])+'\u5957)','history':h})
    else:
        record_scan(bid[0], code, worker, 'wrong', 0, region or '', 'N/A')
        c.execute('SELECT doc_number,worker,result,expected_qty,scanned_at,region,note FROM scans WHERE batch_id=? AND region=? ORDER BY id DESC LIMIT 20', (bid[0], region or ''))
        h = [{'code':h[0],'worker':h[1],'status':'\u2713' if h[2]=='correct' else '\u2717','qty':h[3],'time':h[4],'note':h[6] if h[6] else ''} for h in c.fetchall()]
        conn.close()
        if item and item[0] == 'wrong_region':
            return _json_response({'found':False,'history':h,'message':'\u2717 '+code+'\u5df2\u5206\u914d\u5230 '+item[1]})
        return _json_response({'found':False,'history':h,'message':'\u2717 '+code+'\u672a\u627e\u5230\u5bf9\u5e94\u8bb0\u5f55'})

@app.route('/scan_stats')
def route_scan_stats():
    q = request.args
    bid = q.get('batch','')
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    if bid and bid.isdigit() and int(bid) > 0:
        c.execute('SELECT COUNT(*),SUM(CASE WHEN result=\'correct\' THEN 1 ELSE 0 END) FROM scans WHERE batch_id=?', (int(bid),))
    else:
        c.execute('SELECT COUNT(*),SUM(CASE WHEN result=\'correct\' THEN 1 ELSE 0 END) FROM scans')
    row = c.fetchone(); conn.close()
    return _json_response({'total':row[0] or 0,'correct':row[1] or 0})

@app.route('/scan_batches')
def route_scan_batches():
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute('SELECT id,name,created_at,status,items_count FROM batches ORDER BY id DESC LIMIT 20')
    batches = c.fetchall()
    result = []
    for b in batches:
        c.execute('SELECT COUNT(*),SUM(CASE WHEN result=\'correct\' THEN 1 ELSE 0 END) FROM scans WHERE batch_id=?', (b[0],))
        stats = c.fetchone()
        result.append({'id':b[0],'name':b[1],'created_at':b[2],'status':b[3],'items_count':b[4],'total_scans':stats[0] or 0,'correct':stats[1] or 0,'wrong':(stats[0] or 0)-(stats[1] or 0)})
    conn.close()
    return _json_response(result)

@app.route('/workshop_data')
def route_workshop_data():
    q = request.args
    bid = q.get('batch','0')
    default_ppl = int(q.get('people','0')) or 0
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    if bid and bid.isdigit() and int(bid) > 0:
    ...
    for i in c.fetchall():
        ...
        actual_worker = item['worker'] if item['worker'] else ('x' + str(default_ppl) if default_ppl > 0 else 'x1')
...
@app.route('/calc_capacity')
def route_calc_capacity():
    sku = request.args.get('sku','').strip().upper()
    if sku:
        result = calc_efficiency(sku)
        return _json_response(result if result else {'error':'\u65e0\u6570\u636e'})
    return _json_response({'error':'\u8bf7\u63d0\u4f9bSKU'})

@app.route('/get_efficiency')
def route_get_efficiency():
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute('SELECT sku,rate,note,created_at FROM efficiency ORDER BY sku')
    rows = [{'sku':r[0],'rate':r[1],'note':r[2] or '','created_at':r[3]} for r in c.fetchall()]
    conn.close()
    return _json_response(rows)

@app.route('/get_job_efficiency')
def route_get_job_efficiency():
    sku = request.args.get('sku','').strip().upper()
    if sku:
        result = get_job_efficiency(sku)
        return _json_response({'sku':sku,'data':result} if result else {'sku':sku,'data':None})
    return _json_response({'error':'\u8bf7\u63d0\u4f9bSKU'})

@app.route('/job_batches')
def route_job_batches():
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute('SELECT DISTINCT substr(job_number,1,8) as dt FROM job_items WHERE job_number IS NOT NULL ORDER BY dt DESC')
    batches = [r[0] for r in c.fetchall()]
    conn.close()
    return _json_response(batches)

@app.route('/scan_history')
def route_scan_history():
    q = request.args
    bid = q.get('batch','')
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    if bid and bid.isdigit():
        c.execute('SELECT doc_number,worker,result,expected_qty,scanned_at,region,note FROM scans WHERE batch_id=? ORDER BY id DESC LIMIT 100', (int(bid),))
    else:
        c.execute('SELECT doc_number,worker,result,expected_qty,scanned_at,region,note FROM scans ORDER BY id DESC LIMIT 100')
    rows = [{'code':r[0],'worker':r[1],'status':'\u2713' if r[2]=='correct' else '\u2717','qty':r[3],'time':r[4],'region':r[5],'note':r[6] or ''} for r in c.fetchall()]
    conn.close()
    return _json_response(rows)
...
def route_track():
    ...
    result = track_logistics(num)
    return _json_response(result)

@app.route('/q')
def route_q():
    query = request.args.get('q','').strip()
    if not query: return _json_response({'status':'error','message':'\u8bf7\u8f93\u5165\u95ee\u9898'})
    result = answer_query(query)
    return _json_response({'status':'ok','message':result})

# ---- POST /run - main action handler ----
    ...
    try:
        ...
        if action == 'start_job':
            ...
            if item_id:
                ...
                conn.commit(); conn.close()
                return _json_response({'status':'ok','message':'\u2705 \u5df2\u5f00\u59cb\u52a0\u5de5'})
            return _json_response({'status':'error','message':'\u274c \u65e0\u6548ID'})
        
        if action == 'complete_job':
            item_id = int(batch_name) if batch_name.isdigit() else 0
            done_qty = request.form.get('done_qty','0')
            ...
            if item_id:
                ...
                conn.commit(); conn.close()
                return _json_response({'status':'ok','message':'\u2705 \u5df2\u5b8c\u6210'})
            return _json_response({'status':'error','message':'\u274c \u65e0\u6548ID'})
        ...
        if action == 'cancel_job':
            ...
            if item_id:
                ...
                conn.commit(); conn.close()
                return _json_response({'status':'ok','message':'\u2705 \u5df2\u53d6\u6d88\u52a0\u5de5\uff0c\u53ef\u91cd\u65b0\u5f00\u59cb'})
            return _json_response({'status':'error','message':'\u274c \u65e0\u6548ID'})
        ...
        if action == 'delete_jobs':
            ...
            if ids_str:
                ...
                if ids:
                    ...
                    conn.commit(); conn.close()
                    return _json_response({'status':'ok','message':'\u2705 \u5df2\u5220\u9664 ' + str(len(ids)) + ' \u9879'})
            return _json_response({'status':'error','message':'\u274c \u65e0\u6548\u7684ID\u5217\u8868'})
        
        if action == 'set_priority':
            iid = int(request.form.get('batch_name', 0))
            pri = int(request.form.get('priority', 0))
            conn = sqlite3.connect(DB_PATH); c = conn.cursor()
        ...
        if action == 'pause_job':
            ...
            if iid:
                ...
                conn.commit(); conn.close()
                return _json_response({'status':'ok','message':'\u23f8 \u5df2\u6682\u505c'})
            return _json_response({'status':'error','message':'\u274c \u65e0\u6548ID'})
        ...
        if action == 'resume_job':
            ...
            if iid:
                ...
                if r:
                    ...
                    conn.commit(); conn.close()
                    return _json_response({'status':'ok','message':'\u25b6 \u5df2\u6062\u590d'})
                conn.close()
                return _json_response({'status':'error','message':'\u274c \u65e0\u6548ID'})
        ...
        if action == 'complete':
            ...
            if bid:
                ...
                conn.commit(); conn.close()
                return _json_response({'status':'ok','message':'\u2705 \u5df2\u786e\u8ba4\u5b8c\u6210\u6279\u6b21'})
            return _json_response({'status':'error','message':'\u274c \u65e0\u6548\u7684\u6279\u6b21ID'})
        
        if action == 'import_jobs':
            res = import_jobs(save_path, batch_name)
            return _json_response({'status':'ok','message':res})
        ...
        if action == 'save_efficiency':
            ...
            if sku and rate:
                try:
                    ...
                    conn.commit(); conn.close()
                    return _json_response({'status':'ok','message':sku+' = '+str(rate)+' \u5df2\u4fdd\u5b58'})
                except Exception as e:
                    return _json_response({'status':'error','message':str(e)})
            return _json_response({'status':'error','message':'\u53c2\u6570\u9519\u8bef'})
        ...
        if action == 'delete_efficiency':
            ...
            if sku:
                ...
                conn.commit(); conn.close()
                return _json_response({'status':'ok','message':'\u5df2\u5220\u9664'})
            return _json_response({'status':'error','message':'\u53c2\u6570\u9519\u8bef'})
        ...
        return _json_response({'status':'error','message':'Unknown action: '+action})
    except Exception as e:
        import traceback
        traceback.print_exc()
        return _json_response({'status':'error','message':str(e)})
...
if __name__ == '__main__':
    ...
    print('='*45)
    print('  \u5de5\u5382\u5de5\u4f5c\u53f0 v3.2 (Flask+Waitress)')
    print('  '+url)
