...
# ====== 数据库 ======
DB_PATH = os.path.join(os.path.expanduser('~'), 'Desktop', '_scan_data.db')
def init_db():
...
def import_jobs(fp, batch_name):
    ...
    for r in range(2, ws.max_row + 1):
        ...
        c.execute('INSERT INTO job_items (batch_id, sku, product_name, qty) VALUES (?,?,?,?)', (bid, sku.upper(), name, qty))
        cnt += 1
...
.hd{display:none}#reader{width:100%;max-width:400px;margin:0 auto;position:relative}
.cr{font-size:11px;color:#999;text-align:center;margin-top:15px}
/* 闪光灯按钮 */
.flash-btn{position:absolute;top:10px;right:10px;z-index:10;width:44px;height:44px;border-radius:50%;border:2px solid rgba(255,255,255,.8);background:rgba(0,0,0,.4);color:#fff;font-size:20px;cursor:pointer;display:none;align-items:center;justify-content:center}
.flash-btn.on{background:rgba(255,193,7,.6);border-color:#ffc107}
/* 点击启动提示层 */
.start-hint{position:absolute;top:0;left:0;right:0;bottom:0;z-index:5;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(0,0,0,.85);color:#fff;cursor:pointer;border-radius:8px}
.start-hint .icon{font-size:48px;margin-bottom:10px}
.start-hint .txt{font-size:16px;color:#ccc}.start-hint .sub{font-size:12px;color:#888;margin-top:4px}
/* 继续扫码悬浮按钮 */
.next-btn{position:fixed;bottom:30px;left:50%;transform:translateX(-50%);padding:12px 32px;background:#1a73e8;border:none;border-radius:25px;color:#fff;font-size:16px;z-index:20;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.3);display:none}
</style></head><body>
<h1>📷 扫码核对</h1>
<p class="st" id="batchInfo">加载中...</p>
<select class="sel" id="regionSel" onchange="autoStart()"><option value="">-- 请选择区域 --</option></select>
<input class="i" id="worker" placeholder="输入你的名字" oninput="localStorage.setItem('sw',this.value);autoStart()">
<div id="reader"><div id="flashContainer"></div></div>
<button class="next-btn" id="nextBtn" onclick="restartCamera()">📷 继续扫码</button>
...
<script>
// VER=v7
let qr=null,scanning=false,flashOn=false,camTrack=null;
var w=localStorage.getItem('sw');if(w)document.getElementById('worker').value=w;

function autoStart(){
    var wn=document.getElementById('worker').value.trim();
    var rg=document.getElementById('regionSel').value;
    if(wn && rg && !qr){startScan();}
}
...
async function startScan(){
    ...
    localStorage.setItem('sw',wn);localStorage.setItem('sr',rg);
    document.getElementById('reader').innerHTML='<div id="flashContainer"></div>';
    document.getElementById('reader').style.display='block';
    qr=new Html5Qrcode("reader");
    try{
        await qr.start({facingMode:"environment"},{fps:10,qrbox:{width:250,height:250}},onScan);
        // 尝试获取摄像头轨道用于闪光灯
        try{
            var cams=Html5Qrcode.getCameras();
            // 闪光灯按钮加在reader上
            var fb=document.createElement('div');
            fb.id='flashBtn';fb.className='flash-btn';fb.innerHTML='\u2600';
            fb.onclick=toggleFlash;
            document.getElementById('flashContainer').appendChild(fb);
            fb.style.display='flex';
        }catch(e){}
    }catch(e){
        // 首次启动需要用户交互 - 显示点击提示
        qr=null;
        document.getElementById('reader').innerHTML='<div class="start-hint" onclick="startScan()"><div class="icon">📷</div><div class="txt">点击启动相机</div><div class="sub">'+wn+' · '+rg+'</div></div>';
    }
}
...
async function toggleFlash(){
    ...
    try{
        flashOn=!flashOn;
        await qr.setTorch(flashOn);
        document.getElementById('flashBtn').className='flash-btn'+(flashOn?' on':'')+'';
    }catch(e){alert('\u95ea\u5149\u706f\u4e0d\u652f\u6301');flashOn=false}
}

function onScan(c){if(scanning)return;scanning=true;qr.stop();camTrack=null;document.getElementById('reader').style.display='none';document.getElementById('nextBtn').style.display='none';check(c);}
...
WORKSHOP_PAGE = '''<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"><title>车间加工</title><style>
*{margin:0;padding:0;box-sizing:border-box}
...
.cr{font-size:11px;color:#999;text-align:center;padding:12px;margin-top:8px}
	...
	.pri-item{background:#fffaf0;border-left:3px solid #e53e3e;margin-bottom:6px!important}</style></head><body>
				<div class="hd"><div style="display:flex;align-items:center;gap:8px"><h1 style="flex:1">🔧 车间加工列表 (手机端)</h1><a href="#" style="color:#fff;text-decoration:none;font-size:18px" onclick="showQR();return false">📱</a></div><p id="batchInfo">全部加工单</p></div>
			...
			<div style="margin:0 12px 4px;padding:6px 10px;background:#fffbeb;border:1px solid #f6e05e;border-radius:6px;font-size:11px;color:#975a16">📢 开始、暂停、完工环节，须第一时间点击对应按键</div>
		...
		<div class="people-bar"><a href="#" style="text-decoration:none;font-size:15px;margin-right:6px" onclick="startScan();return false">📷 扫码</a><span>👥</span><input id="peopleInput" type="number" min="1" placeholder="人数" onchange="savePeople()"><span style="font-size:11px;color:#999" id="peopleLabel">上班: -</span></div>
...
async function loadJobs(){
    ...
    if(el){if(priPending>0){el.style.display='block';document.getElementById('priCount').textContent=priPending}else el.style.display='none'}
	    ...
	    if(priPending>0 && (!window.lastPriCount || priPending>window.lastPriCount) && !window.priAlertDismissed){
...
function autoRefresh(){loadJobs();setTimeout(autoRefresh,2000);}
	autoRefresh();
		...
		// 扫码（使用 QuaggaJS，兼容所有手机浏览器）
		var scanRunning=false;
		function startScan(){
		    document.getElementById('scanOverlay').style.display='flex';
		    var v=document.getElementById('scanVideo');
		    // 先尝试原生BarcodeDetector（更快）
		    if('BarcodeDetector' in window && location.protocol==='https:'){
		        navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}}).then(function(s){
		            scanRunning=true;v.srcObject=s;v.play();
		            (function loop(){
		                if(!scanRunning) return;
		                if(!v.videoWidth){setTimeout(loop,300);return}
		                new BarcodeDetector({formats:['ean_13','ean_8','code_128','code_39']}).detect(v).then(function(c){if(c.length>0)sF(c[0].rawValue);else setTimeout(loop,300)}).catch(function(){setTimeout(loop,300)});
		            })();
		        }).catch(function(){startQuagga()});
		    }else startQuagga();
		}
		function startQuagga(){
		    ...
		    if(!window.Quagga){
		        ...
		        s.src='https://cdn.jsdelivr.net/npm/quagga/dist/quagga.min.js';
		        s.onload=function(){runQuagga()};
		...
		function runQuagga(){
		    ...
		    scanRunning=true;
		    Quagga.init({
		        inputStream:{name:'Live',type:'LiveStream',target:document.getElementById('scanVideo'),constraints:{width:640,height:480,facingMode:'environment'}},
		        decoder:{readers:['ean_reader','ean_8_reader','code_128_reader','code_39_reader']}
		    },function(err){
		        if(err){scanRunning=false;scanFallback();return}
		        Quagga.start();
		        Quagga.onDetected(function(data){
		            if(data&&data.codeResult&&data.codeResult.code){
		                Quagga.stop();scanRunning=false;
		                sF(data.codeResult.code);
		            }
		        });
		    });
		}
		function scanFallback(){
		    var fi=document.getElementById('scanCapture');
		    fi.onchange=function(){
		        if(fi.files.length){
		            var img=new Image();
		            img.onload=function(){
		                if('BarcodeDetector' in window){
		                    new BarcodeDetector({formats:['ean_13','ean_8','code_128','code_39']}).detect(img).then(function(c){if(c.length>0)sF(c[0].rawValue);else alert('⚠ 未识别到条码');fi.value=''}).catch(function(){alert('⚠ 识别失败');fi.value=''});
		                }else alert('⚠ 浏览器不支持扫码');
		            };
		            var r=new FileReader();r.onload=function(e){img.src=e.target.result};r.readAsDataURL(fi.files[0]);
		        }
		    };
		    fi.click();
		}
		...
		function scanFromImage(fi){
		    if(!fi.files.length) return;
		    var img=new Image();
		    img.onload=function(){
		        if('BarcodeDetector' in window){
		            var d=new BarcodeDetector({formats:['ean_13','ean_8','code_128','code_39']});
		            d.detect(img).then(function(c){if(c.length>0)sF(c[0].rawValue);else alert('⚠ 未识别到条码，请手动输入');fi.value=''}).catch(function(){alert('⚠ 识别失败，请手动输入');fi.value=''});
		        }else{alert('⚠ 当前浏览器不支持扫码，请输入条码');fi.value=''}
		    };
...
<div id="scanOverlay" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;z-index:200;background:#000" onclick="stopScan()">
    <video id="scanVideo" style="width:100%;height:100%;object-fit:cover" autoplay playsinline></video>
    <div id="scanSku" style="position:absolute;top:20px;left:0;right:0;text-align:center;font-size:20px;font-weight:700;color:#fff;text-shadow:0 2px 4px rgba(0,0,0,.5);z-index:201"></div>
    <div style="position:absolute;bottom:40px;left:0;right:0;text-align:center;z-index:201">
        <button onclick="stopScan()" style="background:rgba(255,255,255,.2);color:#fff;border:2px solid #fff;border-radius:50%;width:60px;height:60px;font-size:24px;cursor:pointer">✕</button>
    </div>
    <input type="file" id="scanCapture" accept="image/*" capture="environment" style="display:none">
</div>
		</body></html>'''
...
.cb-item{width:14px;height:14px;cursor:pointer;margin-right:4px;flex-shrink:0}</style></head><body>
	<div class="hd" style="display:flex;align-items:center;gap:10px"><a href="/" style="color:#fff;text-decoration:none;font-size:14px;font-weight:600;background:rgba(255,255,255,.15);padding:4px 10px;border-radius:6px;white-space:nowrap">🏠 主页</a><div><h1>🛡 管理后台</h1><p id="batchInfo">全部加工单</p><p id="debugInfo" style="font-size:10px;color:#ff0;text-align:center">加载中...</p></div></div>
    ...
    items.forEach(function(i){
        ...
        var stClass = i.status;
            ...
            var btns='';
                ...
                else if(i.status==='processing'){btns='<button class="btn-c" onclick="openComplete(this)">✔ 完成</button><button class="btn-x" style="background:#d69e2e" onclick="pauseJob('+i.id+')">⏸ 暂停</button><button class="btn-x" onclick="cancelJob('+i.id+')">✖ 取消</button>';}
            ...
            if(i.status==='completed' && i.done_qty) info+='<div class="qt">完成: '+i.done_qty+'件 | '+i.completed.substr(11,5)+'</div>';
            var priBadge = i.priority ? '<span class="pri-badge">⭐优先</span>' : '';
            li.innerHTML='<div class="top"><input type="checkbox" class="cb-item" data-id="'+i.id+'" onchange="updateSelCount()"><span class="st '+stClass+'"></span><span class="sk">'+i.sku+priBadge+'</span><span style="font-size:10px;color:#999">'+statusLabel[i.status]+'</span></div>'+'<div class="nm">'+(i.name||'')+'</div>'+info+'<div class="btns">'+priBtn+btns+'</div>';
        ul.appendChild(li);
    });
}
async function startJob(id){
...
.empty{text-align:center;font-size:12px;color:#a0aec0;padding:16px 0}
	...
	.cb-item{width:14px;height:14px;cursor:pointer;margin-right:4px;flex-shrink:0}</style></head><body>
		<div class="hd"><div class="hd-top">
			<div style="display:flex;align-items:center;gap:10px"><a href="/" style="color:#fff;text-decoration:none;font-size:14px;font-weight:600;background:rgba(255,255,255,.15);padding:4px 10px;border-radius:6px;white-space:nowrap">🏠 主页</a><div><h1>🔧 车间看板</h1><div class="sub" id="batchInfo">加载中...</div></div></div>
						<div class="people-bar"><a class="nav-link" href="/workshop" target="_blank">📱列表</a><a class="nav-link" href="/workshop_admin" target="_blank">🛡管理</a></div>
		</div></div>
...
class H(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if p.startswith('/workshop_data'):
            ...
            if bid and bid.isdigit():
                c.execute('SELECT id,sku,product_name,qty,customer,notes,status,worker,started_at,completed_at,completed_qty,priority FROM job_items WHERE batch_id=? ORDER BY priority DESC, id', (int(bid),))
            else:
                c.execute('SELECT id,sku,product_name,qty,customer,notes,status,worker,started_at,completed_at,completed_qty,priority FROM job_items ORDER BY priority DESC, id DESC')
            items = []
            for i in c.fetchall():
                item = {'id':i[0],'sku':i[1],'name':i[2],'qty':i[3],'customer':i[4],'notes':i[5],'status':i[6],'worker':i[7] or '','started':i[8] or '','completed':i[9] or '','done_qty':i[10] or 0,'priority':i[11] if i[11] else 0}
                # 用实际人数或默认人数来估算预计用时
                if item['worker']:
                    ...
                    if item['status'] == 'processing' and item['started']:
                        ...
                        if remain_min < 600:
                            try:
                                ...
                                est_time = started + datetime.timedelta(minutes=remain_min)
    ...
    def do_POST(self):
        try:
            ...
            if action in ('start_job', 'complete_job', 'set_priority', 'cancel_job', 'delete_jobs', 'pause_job', 'resume_job'):
                pass  # handle below
                ...
                if item_id:
                    ...
                    if started:
                        ...
                        try:
                            ...
                            e = dt.datetime.fromisoformat(now)
                            mins = int((e-s).total_seconds()/60)
                            msg += f'\n\u23f1 {mins}分钟'
                            if done_qty > 0:
                                rate = done_qty/(max(mins,1)/60)
                                msg += f'\n\u2699 {done_qty}/单/小时: {round(rate,2)}'
                        except: pass
...
def run_lbl100(fp):
    ...
    for idx,bn_ in enumerate(sorted(boxes.keys()),1):
        for ps in range(0, len(boxes[bn_]), 6):
            ...
            lh += '<div class="l"><div class="bh">(\u7b2c'+str(idx)+'\u7bb1/\u603b'+str(tb)+'\u7bb1)</div><table><tr><th>SKU</th><th>\u54c1\u540d</th><th>\u578b\u53f7</th><th>\u5305\u6570</th><th>\u5355\u5305\u6570\u91cf</th><th>\u91c7\u8d2d\u91cf</th><th>\u7bb1\u53f7</th></tr>'
            for sku,name,model,pk,qty,total in ch:
                lh += '<tr><td style="font-size:10px">'+sku+'</td><td style="font-size:12px">'+name+'</td><td style="font-size:10px">'+model+'</td><td style="font-size:12px;text-align:center">'+pk+'</td><td style="font-size:12px;text-align:center">'+qty+'</td><td style="font-size:12px;text-align:center">'+total+'</td><td style="font-size:12px;text-align:center">'+str(idx)+'</td></tr>'
            lh += '</table></div>'
    s = '<style>@page{size:100mm 100mm;margin:0}body{font-family:"Microsoft YaHei","PingFang SC",sans-serif;margin:0;padding:0}.l{width:100mm;height:100mm;padding:1.5mm;page-break-after:always;overflow:hidden;display:flex;flex-direction:column}.l:last-child{page-break-after:auto}.bh{text-align:center;font-weight:bold;font-size:11px;padding:1mm 0}table{width:100%;border-collapse:collapse;table-layout:fixed}th,td{padding:0.8mm 0.5mm;border:0.7px solid #000;line-height:1.15;word-break:break-all}th{font-size:9px;text-align:center}th:nth-child(1),td:nth-child(1){width:18%}th:nth-child(2),td:nth-child(2){width:28%}th:nth-child(3),td:nth-child(3){width:14%}th:nth-child(4),td:nth-child(4){width:8%}th:nth-child(5),td:nth-child(5){width:10%}th:nth-child(6),td:nth-child(6){width:10%}th:nth-child(7),td:nth-child(7){width:12%}.np{text-align:center;padding:8px;background:#fff3cd;border-bottom:2px solid #ffc107}@media print{.np{display:none}}</style>'
    ...
    fout = os.path.join(DESKTOP, ff+'-'+'\u6807\u7b7e-'+nc+'.html')
...
def run_lbl30(fp):
    ...
    fout = os.path.join(DESKTOP, '\u6807\u7b7e-\u63d0\u8d27\u4fe1\u606f.html')
    with open(fout,'w',encoding='utf-8') as f: f.write(html)
...
def run_us(fp):
    ...
    fout=os.path.join(DESKTOP,os.path.basename(fp).replace('.xlsx','-\u6c47\u603b.xlsx'))
    out.save(fout)
...
def run_ca(fp):
    ...
    fout=os.path.join(DESKTOP,os.path.basename(fp).replace('.xlsx','-\u52a0\u62ff\u5927\u6c47\u603b.xlsx'))
    out.save(fout)
    return '\u2705 \u52a0\u62ff\u5927\u53d1\u8d27\u6c47\u603b\n\u5408\u8ba1\uff1a'+str(len(d))+'\u5355\uff0c'+str(gb)+'\u7bb1\uff0c'+str(round(gw,2))+'kg\uff0c'+str(round(gv,2))+'m3\n\u6587\u4ef6\uff1a'+os.path.basename(fout)

def run_rc(fp):
    import openpyxl; from collections import defaultdict
    wb=openpyxl.load_workbook(fp,data_only=True); ws=wb['Sheet2']
    left={}
    for r in range(3,ws.max_row+1):
        sku=str(ws.cell(r,2).value or '').strip(); qty=ws.cell(r,4).value
        if sku and qty is not None:
            try: left[sku]=left.get(sku,0)+int(float(str(qty)))
            except: pass
    right={}
    for r in range(3,ws.max_row+1):
        sku=str(ws.cell(r,6).value or '').strip(); qty=ws.cell(r,8).value
        if sku and qty is not None:
    ...
    try:
        ...
        for r in rows:
            try:
                ...
                total_seconds += max((e-s).total_seconds(), 0)
            except: pass
    ...
    try:
        ...
        if rows:
            ...
            for prev_qty, prev_end, prev_start, prev_worker, paused_sec in rows:
                ...
                dur_min = max((e - s).total_seconds() - (paused_sec or 0), 60) / 60  # 扣除暂停时间
